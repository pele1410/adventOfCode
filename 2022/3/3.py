...
def letter_to_score(letter_):
    """
    Lowercase letters are 1-26
    Uppercase letters are 27-52
    """
    offset = 96 if letter_.lower() == letter_ else 38
    return ord(letter_) - offset


def split_line_into_compartments(line_):
    length = len(line_)
    half = (length // 2 if length % 2 == 0 else length // 2 + 1)
    compartments = [0, 0]
    compartments[0] = line_[0: half]
    compartments[1] = line_[half:]
    return compartments
...
def total_score_part_1(lines_):
    total_score = 0

    for line in lines_:
        compartments = split_line_into_compartments(line)
        common = compartment_intersection(compartments)
...
def main():
    ...
    print("Part 1:", total_score_part_1(lines))
    print("Part 2:", total_score_part_2(lines))
